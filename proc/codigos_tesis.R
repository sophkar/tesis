...
base_princ <- docente_fil_r %>%
  transmute(
    ...
    mrun           = as.factor(mrun),
    RBD            = as.factor(RBD),
    AGNO           = as.integer(AGNO)              # numérico (no factor por ahora)
  ) %>%
  tidyr::drop_na(optimo) %>%
  droplevels()

base_princ %>% summarise(n = n(), n_mrun = n_distinct(mrun), n_RBD = n_distinct(RBD), min_year=min(AGNO), max_year=max(AGNO))
...
base_princ <- docente_fil_r %>%
  ...
  tidyr::drop_na(optimo) %>%
  mutate(
    AGNO_c = scale(AGNO, center = TRUE, scale = FALSE) |> as.numeric()
  ) %>%
  droplevels()

fit_fe <- glm(optimo ~ DOC_GENERO + categoria_peda + dependencia + AGNO_c,
              family = binomial, data = base_princ)
start_fixef <- coef(fit_fe)

fit_m1 <- glmer(
  optimo ~ DOC_GENERO + categoria_peda + dependencia + AGNO_c +
    (1 | mrun),
  family = binomial, data = base_princ,
...
base_ml <- docente_fil_r %>%
  ...
  mutate(
    ...
    AGNO          = as.integer(AGNO),
    AGNO_c        = as.numeric(scale(AGNO, center = TRUE, scale = FALSE))
  ) %>%
  tidyr::drop_na(optimo) %>%
  droplevels()
