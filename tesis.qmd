...
En el modelo multinivel de probabilidad de egreso, se observa que las mujeres presentan 1,87 veces más probabilidades de titularse en comparación con los hombres, manteniendo constantes las demás variables del modelo.

Respecto a la categoría pedagógica, los estudiantes de pedagogía básica muestran menores probabilidades de egreso que quienes estudian pedagogía parvularia (categoría de referencia), con un odds ratio de 0,61, lo que representa una reducción cercana al 39% en sus chances de culminar los estudios. En cambio, la categoría mixta no presenta diferencias relevantes respecto a parvularia (OR = 1,00).

El tipo de institución formadora también muestra diferencias significativas. En relación con los institutos profesionales (categoría de referencia), los estudiantes que provienen de universidades estatales tienen 1,54 veces más probabilidades de egresar, mientras que en las universidades privadas el efecto es aún mayor, con un odds ratio de 1,71.

En términos de varianza, el modelo reporta un ICC de 0,07, lo que indica que aproximadamente un 7% de la variabilidad en la probabilidad de egreso se explica por diferencias entre instituciones, más allá de las características individuales.
...
En comparación con los hombres, las mujeres presentan menores probabilidades de alcanzar un ejercicio docente óptimo (OR = 0,80). En relación con la categoría de referencia (pedagogía básica), quienes provienen de programas mixtos muestran una probabilidad aún menor (OR = 0,52), mientras que los egresados de pedagogía parvularia presentan la probabilidad más baja de todas (OR = 0,32).

Al analizar la dependencia administrativa, tomando como referencia los establecimientos municipales, los docentes que se desempeñan en el sector particular subvencionado tienen un 63% menos de probabilidades de alcanzar un ejercicio óptimo (OR = 0,37), y aquellos en el sector particular pagado una reducción similar (OR = 0,35).

Por otra parte, el efecto del tiempo resulta positivo: por cada año adicional en la trayectoria laboral, la probabilidad de lograr un ejercicio óptimo aumenta en torno a un 25% (OR = 1,25).

En términos de variabilidad, el modelo arroja un ICC de 0,65, lo que indica que más de la mitad de la variación en la probabilidad de ejercicio docente óptimo se explica por diferencias entre individuos, incluso después de controlar por características institucionales y contextuales.
...
Las mujeres presentan una menor probabilidad de alcanzar un ejercicio docente óptimo en comparación con los hombres (OR = 0,86), lo que evidencia una brecha de género persistente, aun considerando la heterogeneidad entre instituciones de formación. En cuanto a la categoría pedagógica, los egresados de programas mixtos muestran una probabilidad reducida de desempeño óptimo (OR = 0,47) y los de parvularia presentan el efecto más marcado, con una disminución del 80% en comparación con quienes egresan de pedagogía básica (OR = 0,20).

Las diferencias también se expresan en la dependencia del establecimiento laboral: en relación con el sector municipal, los docentes en particulares subvencionados exhiben una probabilidad cinco veces menor de alcanzar un desempeño óptimo (OR = 0,20), mientras que en los particulares pagados la probabilidad se reduce en torno a dos tercios (OR = 0,34).

El efecto del tiempo sigue siendo consistente: cada año adicional en la trayectoria laboral se asocia con un aumento del 32% en la probabilidad de desempeño óptimo (OR = 1,32). Finalmente, el modelo arroja un ICC de 0,77, lo que indica que una proporción relevante de la variabilidad se explica tanto por diferencias entre trayectorias individuales como entre instituciones formadoras, reforzando la necesidad de considerar simultáneamente ambos niveles para comprender la heterogeneidad en el ejercicio docente.
